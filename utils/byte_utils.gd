...
static func flip_bit_by_index(idx: int, bytes: PackedByteArray) -> int:
	var byte_idx: int = which_byte_is_bit_in(idx)
	bytes[byte_idx] = get_byte_with_flipped_bit_index(get_bit_offset_within_byte(idx),bytes[byte_idx])
	return bytes[byte_idx]
...
static func serialize_v2(v2: Vector2) -> PackedByteArray:
	var bytes: PackedByteArray=  []
	bytes.resize(8)
	return encode_v2(bytes,v2,0)
...
static func encode_r2(bytes: PackedByteArray, r2: Rect2, offset: int) -> PackedByteArray:
	encode_v2(bytes,r2.position,offset)
	encode_v2(bytes,r2.size,offset+4)
	return bytes

static func decode_r2(bytes: PackedByteArray, offset: int) -> Rect2:
	return Rect2(
		bytes.decode_float(offset),
		bytes.decode_float(offset+4),
		bytes.decode_float(offset+8),
...
static func encode_r2i(bytes: PackedByteArray, r2i: Rect2i, offset: int) -> PackedByteArray:
	encode_v2i(bytes,r2i.position,offset)
	encode_v2i(bytes,r2i.size,offset+4)
	return bytes

static func decode_r2i(bytes: PackedByteArray, offset: int) -> Rect2i:
	return Rect2i(
		bytes.decode_s32(offset),
		bytes.decode_s32(offset+4),
		bytes.decode_s32(offset+8),
...
static func encode_quat(bytes: PackedByteArray, quat: Quaternion, offset: int) -> PackedByteArray:
	bytes.encode_float(offset,quat.x)
	bytes.encode_float(offset+4,quat.y)
	bytes.encode_float(offset+8,quat.z)
	bytes.encode_float(offset+12,quat.w)
	return bytes

static func decode_quat(bytes: PackedByteArray, offset: int) -> Quaternion:
	return Quaternion(
		bytes.decode_float(offset),
		bytes.decode_float(offset+4),
		bytes.decode_float(offset+8),
...
static func encode_color(bytes: PackedByteArray, color: Color, offset: int) -> PackedByteArray:
...
static func decode_color(bytes: PackedByteArray, offset: int) -> Color:
	return Color(
		bytes.decode_float(offset),
		bytes.decode_float(offset+4),
		bytes.decode_float(offset+8),
...
static func to_binary_string(num: int, Ob: bool = false, min_size: int = 0) -> String:
	var string := ""
	var n : = num
	while n > 0:
		string = str(n&1) + string
		n = n>>1
	assert(string.bin_to_int() == num, "String %s.bin_to_int() number %s != %s."%[string,string.bin_to_int(),num])
	# Why the fuck is this not working?
	#string.lpad(min_size,"0")
	if min_size > string.length():
		for i in min_size - string.length():
			string = "0" + string
	if Ob:
		string = "0b" + string
	return string
...
static func create_bitmask(num_bits: int, on := false) -> PackedByteArray:
	var bitmask := PackedByteArray ()
	bitmask.resize(get_num_bytes_to_hold_num_bits(num_bits))
	if on:
		bitmask.fill(255)
	return bitmask

static func get_bit_count(num: int) -> int:
	return int((log(num) / log(2))) + 1# manually calculate log(2) for gdscript const precision stuff even if it's just 0.693147
