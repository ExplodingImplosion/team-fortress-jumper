...
static func reset_tickrate() -> void:
	var default_tickrate: int = get_default_tickrate()
	if Engine.get_physics_ticks_per_second() != default_tickrate or target_physics_rate != default_tickrate:
		set_physics_simulation_rate(default_tickrate)

const TICKRATE_SETTING_PATH = "physics/common/physics_ticks_per_second"
static func get_default_tickrate() -> int:
	return ProjectSettings.get_setting(TICKRATE_SETTING_PATH,60)
